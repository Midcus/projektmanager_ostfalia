...
<head>
    ...
    <style>
        ...
        .table th:nth-child(1), .table td:nth-child(1) { width: 7%; } /* ID */
        .table th:nth-child(2), .table td:nth-child(2) { width: 30%; font-weight: bold; } /* Name */
        .table th:nth-child(3), .table td:nth-child(3) { width: 10%; font-weight: bold; font-size: 110%; } /* Semester */
        .table th:nth-child(4), .table td:nth-child(4) { width: 10%; } /* Startdatum */
        .table th:nth-child(5), .table td:nth-child(5) { width: 10%; } /* Enddatum */
        .table th:nth-child(6), .table td:nth-child(6) { width: 10%; } /* Vortragdatum */
        .table th:nth-child(7), .table td:nth-child(7) { width: 9.5%; } /* Projektart */
        .table th:nth-child(8), .table td:nth-child(8) { width: 7%; } /* Geheim */
        .table th:nth-child(9), .table td:nth-child(9) { width: 7%; } /* Edit */
    ...
    <div class="main-container">
        <div class="left-column">
            ...
            <div class="card filter-card">
                <div class="card-header-blau">Filter</div>
                <div class="card-body">
                    <form method="GET" action="{{ route('prof.uebersicht') }}">
                        <input type="text" name="name" class="form-control" placeholder="Name" value="{{ request('name') }}">
                        <input type="text" name="semester" class="form-control" placeholder="Semester (z.B SS2024)" value="{{ request('semester') }}">
                        <input type="number" name="startdatum" class="form-control" placeholder="Startdatum (Jahr)" min="1900" max="{{ date('Y') }}" value="{{ request('startdatum') }}">
                        <input type="number" name="enddatum" class="form-control" placeholder="Enddatum (Jahr)" min="1900" max="{{ date('Y') }}" value="{{ request('enddatum') }}">
                        <input type="number" name="vortragdatum" class="form-control" placeholder="Vortragdatum (Jahr)" min="1900" max="{{ date('Y') }}" value="{{ request('vortragdatum') }}">
                        <select name="projektart" class="form-select">
                            <option value="">Projektart</option>
                            <option value="Teamprojekt" {{ request('projektart') === 'Teamprojekt' ? 'selected' : '' }}>Teamprojekt</option>
                            <option value="Studienarbeit" {{ request('projektart') === 'Studienarbeit' ? 'selected' : '' }}>Studienarbeit</option>
                            <option value="Bachelorthesis" {{ request('projektart') === 'Bachelorthesis' ? 'selected' : '' }}>Bachelorthesis</option>
                            <option value="Masterthesis" {{ request('projektart') === 'Masterthesis' ? 'selected' : '' }}>Masterthesis</option>
                        </select>
                        <select name="geheim" class="form-select">
                            <option value="">Geheim</option>
                            <option value="Ja" {{ request('geheim') === 'Ja' ? 'selected' : '' }}>Ja</option>
                            <option value="Nein" {{ request('geheim') === 'Nein' ? 'selected' : '' }}>Nein</option>
                        </select>
                        <div class="filter-buttons">
                            <button type="submit" name="submit" value="1" class="button-24">Suchen</button>
                            <a href="{{ route('prof.uebersicht') }}" class="button-24">Reset</a>
                        </div>
                    </form>
                </div>
            </div>
                    ...
                    <table class="table" id="thesisTable">
                        <thead>
                            <tr>
                                ...
                                <th class="sortable" data-column="2">Semester ⇅</th>
                                <th class="sortable" data-column="3">Startdatum ⇅</th>
                                <th class="sortable" data-column="4">Enddatum ⇅</th>
                                <th class="sortable" data-column="5">Vortragdatum ⇅</th>
                                <th class="sortable" data-column="6">Projektart ⇅</th>
                                <th class="sortable" data-column="7">Geheim ⇅</th> 
                                <th></th>
                            </tr>
                            <tr>
                        ...
                        <tbody>
                            @foreach($theses as $thesis)
                                <tr>
                                    ...
                                    <td>{{ $thesis->semester ?? 'N/A' }}</td>
                                    <td>{{ $thesis->startdatum ? \Carbon\Carbon::parse($thesis->startdatum)->format('d.m.Y') : 'N/A' }}</td>
                                    <td>{{ $thesis->enddatum ? \Carbon\Carbon::parse($thesis->enddatum)->format('d.m.Y') : 'N/A' }}</td>
                                    <td>{{ $thesis->vortragdatum ? \Carbon\Carbon::parse($thesis->vortragdatum)->format('d.m.Y') : 'N/A' }}</td>
                                    <td>
                                    ...
                                    <td>{{ $thesis->geheim == 'yes' ? 'Ja' : 'Nein' }}</td> 
                                    <td>
                                        <div style="display: flex; flex-direction: column; gap: 5px;">
                                            ...
                                            <form action="{{ route('thesis.loeschen', $thesis->id) }}" method="POST" style="display:inline;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger" onclick="return confirm('Sind Sie sicher, dass Sie diese Thesis löschen möchten?');">Löschen</button>
                                            </form>
                                        </div>
                                    </td>
                                </tr>
